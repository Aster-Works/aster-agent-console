...
- **Package metadata points at the renamed GitHub repository**
  (`Aster-Works/aster-agent-audit`). The 0.2.0 tarball still carried the
  pre-rename URLs (harmless — GitHub redirects — but now correct at the
  source). The SARIF `informationUri` follows the same constant.
- README on npm no longer says the package is unpublished, and documents the
  full 0.2.x CLI surface (verify / report / policy / scan formats / baseline).
